...
int main(int argc, char* argv[])
{
    ...
    // main process (here we fill matrix)
    if (myRank == 0)
    {
        // Читаем матрицу из файла
        FILE* inputMatrix;

        inputMatrix = fopen("input.txt", "r");// открыть файл
        start = MPI_Wtime();
        for (i = 0; i < size; i++) // Чтение данных
            for (j = 0; j < size; j++)
                fscanf(inputMatrix, "%d", &matrix1[i * size + j]);

        for (i = 0; i < size; i++)
            for (j = 0; j < size; j++)
                fscanf(inputMatrix, "%d", &matrix2[i * size + j]);
        fclose(inputMatrix);
        ...
        {
            MPI_Send(matrix1 + (l - 1) * line * size, size * line, MPI_INT, l, 1, MPI_COMM_WORLD);
        }
        ...
        inputMatrix = fopen("output.txt", "w");
        for (i = 0; i < size; i++) {
            for (j = 0; j < size; j++)
                fprintf(inputMatrix, "%d ", tmp[i * size + j]);
            fputc('\n', inputMatrix);
        }
        fclose(inputMatrix);
    ...
    // other processes acquire data and send it to main after calcultion is completed
    else
    {
        // get widebroadcasting data (Matrix2)
        MPI_Recv(matrix2, size * size, MPI_INT, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

        MPI_Recv(buf, size * line, MPI_INT, 0, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        // calculate product
        for (i = 0; i < line; i++)
        {
            for (j = 0; j < size; j++)
